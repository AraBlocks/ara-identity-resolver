...
const debug = require('debug')('ara:identity:resolver:cache')
...
class Cache extends EventEmitter {
  ...
  static createDatabase(identifier, opts) {
    ...
    try {
      const db = hyperdb(path, publicKey, {
      ...
      for (const lookup of lookups) {
        ...
        if (result && result.value) {
          ...
          if (entry.ttl >= Date.now()) {
            entry.node = this.parse(entry.value)
            callback(null, entry)
            return
          }
        }
  ...
  async addPeer(identifier, cb) {
    return pify(async (done) => {
      ...
      try {
        const ddo = await aid.resolve(identifier)
        did = new DID(ddo.id)
      } catch (err) {
